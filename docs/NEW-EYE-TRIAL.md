...
## No overlays on the hero at all

The gradients that used to sit over the hero are **gone** - `.stage-veil` and
`.stage-sticky::after` are both `background: none`, and the frame is drawn full strength at
`FIT = 1.0`. The footage is no longer dimmed, tinted or faded anywhere.

That is only safe because the readability was measured rather than assumed. Sampling the
rendered page behind every piece of hero text, across all five states (human eye,
cybernetic eye, burst, globe, world map), with no overlay at all:

| | required | worst measured |
|---|---|---|
| headline | 3.0:1 | 7.8:1 |
| body copy | 4.5:1 | 8.9:1 |
| small mono labels | 4.5:1 | **4.05:1 — failed** |
| button | 4.5:1 | 15.4:1 |

Only the small mono text in the masthead failed, and only over the palest part of the
footage. Dimming the whole image to fix a masthead is the wrong trade, so instead the text
got its own **halo** - a light `text-shadow` that brightens only the pixels touching each
glyph. That took the mono from 4.05:1 to 7.9:1 and the body copy to 10.3:1 while leaving
the image untouched. The button fill went from 60% to 88% opaque for the same reason.

**If the hero copy is ever rewritten or moved, re-measure.** The halo is doing real work
here; there is no overlay left as a safety net.

## The "My Business" label — decided

The clip closes on a world map with a **"My Business"** label burnt into the footage. It is
staying. It cannot be edited out in any case, but it also works in the site's favour: a
visitor reads that pin as *their* business appearing on the map, not as Foresite's. Do not
"fix" it later thinking it is a placeholder.
