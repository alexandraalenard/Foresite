...
- **The hero uses whole, untouched video frames. Nothing is composited.** Three cleverer
  approaches were tried and every one made it worse. Recorded here so they are not tried
  again:

  1. *Cross-fading two gaze frames* to smooth the stepping. They are different eye
     positions, not a tween, so blending them is a double exposure - the pupil and lashes
     ghost against themselves. At rest the eye sits at a fractional position, so it stayed
     permanently doubled: the whole hero looked out of focus after the blink.
  2. *Shifting the whole image* by the sub-step remainder, to keep the eye continuous. The
     eye moves smoothly but the entire picture jerks by ~25px every time the frame changes.
  3. *Compositing a fixed plate behind a moving eye crop*, to hold the HUD still. It does
     hold the HUD still - but lash strands cross the crop boundary and the two frames do not
     line up, so the lashes come out visibly mangled.

  What actually works is the dull answer: draw one whole frame, and only ever step between
  **consecutive** video frames. The HUD then moves by one frame of its own animation, which
  is what the video does when it plays. Measured outside the eye: a 1-frame step changes the
  picture by 5.7 grey levels; the 10-frame gap that used to be in the set changed it by
  11.6, which is why that frame was dropped even though it cost 17px of range.

  **The HUD does still drift slightly as the eye tracks, and that is inherent.** The clip's
  overlay is not stable frame to frame. The only clean way to remove it entirely is a
  re-generated clip with a steady HUD - not more compositing.
